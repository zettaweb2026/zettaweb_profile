...
const TechStackManager = ({ items = [], loading, editingItem, onSave, onDelete, onEdit, onCancel }) => {
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-white/10 bg-black/20 px-6 py-16 text-center text-muted-foreground">
        ...
        <p className="text-sm font-semibold">Loading tech stack...</p>
      </div>
    );
  }
  ...
  if (editingItem) {
    return (
      <form onSubmit={(e) => onSave(e, 'techStack')} className="space-y-6">
  ...
  if (!items.length) {
    return (
      <div className="text-center py-16 text-muted-foreground border border-dashed border-white/10 rounded-2xl bg-black/10">
        ...
        <p className="text-sm font-semibold">No tech stack entries found. Click 'Add New' to create one.</p>
      </div>
    );
  }
      ...
      {items.map((item) => (
        <div key={item.id} className="flex flex-col sm:flex-row justify-between sm:items-center p-5 bg-black/20 hover:bg-white/5 border border-white/5 rounded-2xl transition-all duration-300 group">
          <div className="mb-4 sm:mb-0">
            <h4 className="font-bold text-white text-lg group-hover:text-primary transition-colors">{item.category}</h4>
            <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{Array.isArray(item.technologies) ? item.technologies.length : 0} technologies</p>
          </div>
          <div className="flex gap-2.5 self-end sm:self-auto">
            ...
            <Button size="sm" variant="destructive" onClick={() => onDelete('techStack', item.id)} className="font-semibold rounded-xl px-4 py-1.5 transition-all flex items-center space-x-1">
...
export default React.memo(TechStackManager);
