...
const ServicesManager = ({
  ...
  if (editingItem) {
    return (
      <form onSubmit={(e) => onSave(e, 'services')} className="space-y-6">
  ...
  if (!items.length) {
    return (
      <div className="text-center py-16 text-muted-foreground border border-dashed border-white/10 rounded-2xl bg-black/10">
        ...
        <p className="text-sm font-semibold">No services found. Click 'Add New' to create one.</p>
      </div>
    );
  }
      ...
      {items.map((item) => (
        <div key={item.id} className="flex flex-col sm:flex-row justify-between sm:items-center p-5 bg-black/20 hover:bg-white/5 border border-white/5 rounded-2xl transition-all duration-300 group">
          ...
          <div className="flex gap-2.5 self-end sm:self-auto">
            ...
            <Button size="sm" variant="destructive" onClick={() => onDelete('services', item.id)} className="font-semibold rounded-xl px-4 py-1.5 transition-all flex items-center space-x-1">
...
export default React.memo(ServicesManager);
