...
const UserManager = ({ items = [], loading, editingItem, onSave, onDelete, onEdit, onCancel, selectedPermissions, onPermissionChange }) => {
  if (loading) {
    return (
  ...
  if (editingItem) {
    return (
      <form onSubmit={(e) => onSave(e, 'admins')} className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {[
                ...
                <label key={perm.id} className="flex items-center space-x-3 cursor-pointer p-3 rounded-xl border border-white/5 hover:border-white/10 bg-black/20 hover:bg-white/5 transition-all select-none">
                  <input type="checkbox" checked={selectedPermissions.includes(perm.id)} onChange={() => onPermissionChange(perm.id)} className="rounded border-white/20 text-primary focus:ring-primary/50 bg-black/20 w-4 h-4 cursor-pointer" />
  ...
  if (!items.length) {
    return (
      <div className="text-center py-16 text-muted-foreground border border-dashed border-white/10 rounded-2xl bg-black/10">
        ...
        <p className="text-sm font-semibold">No admin users found. Click 'Add New' to create one.</p>
      </div>
    );
  }
      ...
      {items.map((item) => (
        <div key={item.id} className="flex flex-col sm:flex-row justify-between sm:items-center p-5 bg-black/20 hover:bg-white/5 border border-white/5 rounded-2xl transition-all duration-300 group">
          <div className="mb-4 sm:mb-0">
            ...
            <p className="text-sm text-muted-foreground mt-1 line-clamp-2">Email: {item.email}</p>
            {item.isSuperAdmin ? (
              <span className="text-[10px] px-2 py-0.5 rounded-md font-semibold bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-500/20 text-yellow-400 mt-2 inline-block">Super Admin</span>
            ) : (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {(item.permissions || []).map((permission) => (
                  <span key={permission} className="text-[10px] px-2 py-0.5 rounded-md font-semibold bg-primary/10 border border-primary/10 text-primary uppercase tracking-wider font-mono">{permission}</span>
                ))}
              </div>
            )}
          </div>
          <div className="flex gap-2.5 self-end sm:self-auto">
            ...
            {!item.isSuperAdmin && (
              <Button size="sm" variant="destructive" onClick={() => onDelete('admins', item.id)} className="font-semibold rounded-xl px-4 py-1.5 transition-all flex items-center space-x-1">
